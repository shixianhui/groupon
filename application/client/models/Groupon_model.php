<?php
class Groupon_model extends CI_Model {

	private $_tableName = 'groupon';
	private $_productName = 'product';
    ...
    public function gets($strWhere = NULL, $limit = NULL, $offset = NULL) {
		$ret = array();
		$this->db->select("{$this->_tableName}.*,{$this->_productName}.path,{$this->_productName}.sell_price,{$this->_productName}.market_price");
    ...
    public function get_rand($select = '*', $strWhere = NULL) {
        $ret = array();
        $this->db->select($select);
