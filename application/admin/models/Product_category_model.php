...
class Product_category_model extends CI_Model {
	...
	private $_productVenueTName = 'product_venue';
    ...
    public function gets($strWhere = NULL, $limit = NULL, $offset = NULL) {
		$ret = array();
		$this->db->select("{$this->_tableName}.*, {$this->_productVenueTName}.product_venue_name");
		$this->db->order_by("{$this->_tableName}.sort", 'ASC');
		$this->db->order_by("{$this->_tableName}.id", 'DESC');
		$this->db->join($this->_productVenueTName, "{$this->_tableName}.product_venue_id = {$this->_productVenueTName}.id", 'left');
	...
	public function menuTree() {
		$whereArray = array('parent_id'=>0);
		$menuList = $this->gets($whereArray);
		foreach ($menuList as $key=>$value) {
			$subWhereArray = array('parent_id'=>$value['id']);
			$subMenuList = $this->gets($subWhereArray);
			foreach ($subMenuList as $sKey=>$sValue) {
				$sSubWhereArray = array('parent_id'=>$sValue['id']);
