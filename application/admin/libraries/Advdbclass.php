...
class Advdbclass {	
	...
	public function getPermissionsStr($adminGroupId = NULL) {
	    $ret = false;
	    $CI = & get_instance();
	    $CI->load->model('Admingroup_model', '', TRUE);
	    return $CI->Admingroup_model->get('permissions', array('id'=>$adminGroupId));
	}
	...
	public function getPermissions($adminGroupId = NULL, $permissionsItem = NULL) {
	    ...
	    $CI = & get_instance();
	    $CI->load->model('Admingroup_model', '', TRUE);
	    $admingroupInfo = $CI->Admingroup_model->get('permissions', array('id'=>$adminGroupId));
	    if ($admingroupInfo) {
