<?php
class Quote_price_record extends CI_Controller {
    private $_title = '产品';
    private $_tool = '';
    private $_table = 'quote_price_record';
    private $_template = 'quote_price_record';
    ...
    public function __construct() {
        ...
        $this->_tool = $this->load->view('element/save_list_tool', array('table'=>$this->_table, 'parent_title'=>'商品活动', 'title'=>'报价'), TRUE);
...
//        checkPermission("{$this->_template}_index");
        ...
        if ($_POST) {
            ...
            if ($user_id) {
            	$strWhere .= " and {$this->_table}.mobile = '{$mobile}' ";
            }
        ...
        if ($item_list){
            foreach ($item_list as $key=>$value){
                $user_info = array();
                if ($value['user_id']){
                    $user_info = $this->User_model->getInfo('nickname',array('id'=>$value['user_id']));
                }
                $item_list[$key]['nickname'] = $user_info ? $user_info['nickname'] : '';
            }
        }
